...
class StatefulEditGoods1 extends StatefulWidget {
  final Ndata value;

  const StatefulEditGoods1({Key key, this.value}) : super(key: key);
  @override
  _StatefulEditGoods1State createState() => _StatefulEditGoods1State();
}

class _StatefulEditGoods1State extends State<StatefulEditGoods1> {
  String selectcategory = '';
  String i =
      ''; //keep value of selectcategory because we have to show old category and must show new select category( but new select category is not sure to change).

  //this variable have to get data from backend //picture: i dont know that get data  String or file type, So for example give type is String.
  String picc =
      'https://www.eatforhealth.gov.au/sites/default/files/images/the_guidelines/fruit_selection_155265101_web.jpg';
  File pic; //dont use it
  String name;
  String detail;
  String number;
  String price;
  ...
  Widget build(BuildContext context) {
    ...
    if (selectcategory == '') {
      id = widget.value.id;
      //picc = widget.value.pic;
      picc =
          'https://www.eatforhealth.gov.au/sites/default/files/images/the_guidelines/fruit_selection_155265101_web.jpg';
      name = widget.value.name;
      detail = widget.value.detail;
      catego = widget.value.catego;
      number = widget.value.number;
      price = widget.value.price;
      map = widget.value.map;
      video = widget.value.video;

      namecon.text = name;
      detailcon.text = detail;
      numbercon.text = number;
      pricecon.text = price;
      i = catego;
    }
                                              ...
                                              children: [
                                                Container(
                                                  ...
                                                  child: FlatButton(
                                                    ...
                                                    child: newpic ==
                                                            null //so for example have to show network picture
                                                        ? //Image.file(pic) //if get data pic from back end is File type
                                                        Image.network(
                                                            'https://www.eatforhealth.gov.au/sites/default/files/images/the_guidelines/fruit_selection_155265101_web.jpg') //for example
                                                        : Image.file(newpic),
                                                  ),
                                                )
                                              ],
                        ...
                        Container(
                          ...
                          child: FlatButton(
                            ...
                            onPressed: () async {
                              ...
                              /* pic =
                                  newpic; //Can not change File to String type, So have to gat data pic type is File.
                              name = newname;
                              detail = newdetail;
                              number = newnumber;
                              price = newprice;*/
                              ...
                              try {
                                if (name == null ||
                                  ...
                                  String url =
                                      'http://188.166.251.25:2000/updateproduct';
                                  Map<String, String> headers = {
                                    "Content-type": "application/json"
                                  };
                                  String json =
                                      '{"id": "$id","name":"$name","pic" : "$picc", "detail": "$detail","section": "$catego", "pice": "$number", "price": "$price"}';
                                  // get post delete put
...
class _DelayPageState extends State<DelayPage> {
  ...
  route() {
    Navigator.pushReplacement(
        context, MaterialPageRoute(builder: (context) => StatefulEditGoods1()));
  }
